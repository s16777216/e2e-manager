...
## Purpose
專案基礎設施，包括 Nginx 反向代理、資料庫容器持久化、Shadcn UI 安裝結構與 TypeScript 原生單元測試環境及 Mock 規範。
...
- **WHEN** 測試正在執行，後端 Worker 透過 SSE 載入步驟日誌時
...
- **THEN** 系統不發起真實 SQL 連線，而是直接藉由 Stub 函數回傳模擬的群組節點資料，防環邏輯得出正確計算結果
