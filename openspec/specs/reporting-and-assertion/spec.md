# reporting-and-assertion Specification

## Purpose
步驟截圖、Markdown 報告生成、失敗截圖、以及本地時區轉換與 UTC 時間字串傳輸標準規格。
...
- **THEN** 系統呼叫結構化模型，模型 MUST 回傳包含 `reason`（原因分析）與 `suggestion`（修復建議）這兩個欄位的結構化物件。
- **THEN** 系統將該物件以原生 JSONB 物件格式儲存於資料庫，API 傳輸層直接以 JSON 物件回傳，前端直接進行型別安全的 Bento Card 渲染。

### Requirement: Database JSONB storage for failureSummary
系統的 TestRun 實體中的 `failureSummary` 欄位 MUST 採用 JSONB 資料庫型別，並具有與 Zod Schema 嚴格對齊的 `{ reason: string; suggestion: string }` 結構。
