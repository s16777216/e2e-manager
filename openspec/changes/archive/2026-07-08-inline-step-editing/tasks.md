...
- [x] 2.7 將「新增步驟」按鈕從頂部控制列移除，改在步驟列表下方渲染為 Bento 虛線卡片 (樣式 B)

## 3. 測試與驗證

- [x] 3.1 驗證專案是否能成功編譯，無 TypeScript 與 ESLint 錯誤
- [x] 3.2 於本地瀏覽器驗證既有步驟的修改與即時儲存/取消功能
- [x] 3.3 於本地瀏覽器驗證步驟刪除與順序移動功能
- [x] 3.4 於本地瀏覽器驗證「新增步驟」的暫存狀態，點擊取消時能無縫移除且點擊儲存時才正確寫入資料庫
